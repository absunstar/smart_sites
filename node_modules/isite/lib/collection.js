...
module.exports = function init(____0, option, db) {
  ...
  $collection.deleteMany = $collection.removeMany = $collection.deleteAll = $collection.removeAll = ($options, callback, run) => {
    ...
    if (run !== 2 && run !== !0) {
      ...
      let options = {};
      if ($options.where === undefined) {
        options.where = ____0.copy($options);
      } else {
        options = $options;
      }
  ...
  $collection.import = function (file_path, callback) {
    ...
    if (____0.isFileExistsSync(file_path)) {
      ...
      let docs = JSON.parse(____0.readFileSync(file_path).toString());
      console.log('[ imported file readed ]');
      if (____0.typeof(docs) === 'Array') {
        docs.forEach((doc) => {
  ...
  if (____0.options.mongodb.identity.enabled) {
    ...
    function handleIndex() {
      ...
      if (____0.mongodb.collections_indexed[$collection.collection] === undefined) {
